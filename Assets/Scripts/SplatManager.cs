...
public class SplatManager : MonoBehaviour {
	...
	*/
	...
	IEnumerator UpdateScores() {
		...
		while( true ){
			float _score = CalculateBluePercentage(RenderTextureToTexture2D(scoreTex), 128);
			if (_score > 0) {
				GameControl.Instance.coveredPercent = Mathf.Clamp(_score + Mathf.Lerp(0, 15, _score / 100f), 0f, 100f); //Gives a bit of an error threshold that increases the closer we get to the end.
			}
			GameControl.Instance.ProcessWorldCoveragePoints();
			GameControl.Instance.UpdateCoveredPercent();
